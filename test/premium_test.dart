...
void main() {
  ...
  test(
    'premium dibaca dari cache prefs saat startup (pelindung streak)',
    () async {
  ...
  group('applyServerPremium: server tidak menurunkan premium aktif', () {
    ...
    test(
      'dokumen server ketinggalan (premium: false) → lokal bertahan',
      () async {
        final auth = await authProvider();
    ...
    test(
      'hadiah admin (premiumGrantUntil): memberi, kedaluwarsa, cabut',
      () async {
        final auth = await authProvider();
        ...
        auth.applyServerPremium({
          'premium': false,
          'premiumGrantUntil': DateTime.now().millisecondsSinceEpoch - 1000,
        });
        expect(auth.isPremium, isFalse);
      },
    );

    test('saklar global admin: semua akun premium selama menyala', () async {
      final auth = await authProvider();
      expect(auth.isPremium, isFalse);
    ...
    test('lokal tidak aktif → ikuti server apa adanya', () async {
      ...
      auth2.applyServerPremium({
        'premium': true,
        'premiumUntil': DateTime.now().millisecondsSinceEpoch - 1000,
      });
